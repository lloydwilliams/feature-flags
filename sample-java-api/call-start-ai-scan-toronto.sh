#!/usr/bin/env bash
# Calls StartAIScan for Toronto.
#
# The site is the only thing that differs from call-start-ai-scan-houston.sh:
# run both and the responses show how show-ai-scan is targeted per site.
#
# Override with env vars, e.g. AMOUNT=7 ./call-start-ai-scan-toronto.sh
set -euo pipefail
