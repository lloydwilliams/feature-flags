...
`sample-java-api/logs/sample-java-api.log`. The console and the file use the same
pattern.
...
Unlike JSON logs, a pattern needs parsing on the Datadog side: `source: java` applies the
built-in Java pipeline, but the bracketed trace and span ids are not where it expects
them, so add a grok rule that maps them to `dd.trace_id` and `dd.span_id` if you want
logs and traces linked in the UI.
